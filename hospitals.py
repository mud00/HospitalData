...
st.write('Our analysis is based on hospitals in France and contains information about their locations and specialties, each hospital is represented as a point on the map. The dataset includes hospitals with different specialties, and some hospitals may have multiple specialties separated by semicolons. The interactive map allows filtering the hospitals based on the selected specialty or showcasing all hospitals. Additionally, hospitals without a specified specialty are grouped under the "No Specialty" option.')
