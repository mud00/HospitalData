...
st.write('Our analysis is based on hospitals in France :flag_fr: and contains information about their locations and specialties, each hospital is represented as a point on the map. The dataset includes hospitals with different specialties, and some hospitals may have multiple specialties separated by semicolons. \n\nThe interactive map allows filtering the hospitals based on the selected specialty or showcasing all hospitals.\n\n **Additionally, hospitals without a specified specialty are grouped under the "No Specialty" option.**')
